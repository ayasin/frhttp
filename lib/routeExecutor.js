...
function convertToTemplate(buses, producer, runner, onDef) {
	...
	baconTemplate.onValue(function (fn, name, enter, takeMany, prodObj, val) {
		...
		try {
			...
			console.log('ERROR: enter transform for function (' + name + ') threw an exception (' + err + ') on input: ' + safeJSONStringify(val, null, 2));
			prodObj.error(500, err);
		}
		if (!takeMany) {
			return Bacon.noMore;
...
function writeFile(statusStream, headerSet, writeStream, ender, type, filename, asAttachment) {

...
function attachBus(produced, buses, busName) {
	if (busName.indexOf(':') < busName.length) {
		...
		if (typeof prodVal === 'undefined') {
			buses[busName] = Bacon.constant(null);
		}
		else {
			buses[busName] = Bacon.constant(prodVal);
		}
	}
	else {
...
function buildFnProductionStream(runCountStream, buses, productionStream, def) {
	...
	}, {'default:_noParam' : Bacon.constant(null)});

	var curriedFn = _.curry(def.fn)(makeResponderFnObject(productionStream, buses, runCountStream, outputMap));

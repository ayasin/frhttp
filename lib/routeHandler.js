...
function produceValue(production, obj) {
	procuction.push(obj);
}

function produceEnd(production) {
	production.end();
}
...
/*
	{
		params: ['decode:request', 'dbNames', 'userData'],
...
function buildFnProductionStream(runCountStream, buses, responderObj, def) {
	var inputMap = def.mapInput;
	var outputMap = def.mapProduction;

	var produced = new Bacon.Bus();
	...
	var fnRunStream = Bacon.combineTemplate(template).map(function(runCountStream, val) {
		...
		return val;
	}, inputMap).map(function (val) {
		//TODO something here to fill in real params
		def.fn(val);
	});

	return produced.map(function (outMap, val) {
		if (outMap) {
			return outMap(val);
		}
		return val;
	}, outputMap).map(function (val) {
		runCountStream.push(-1);
	}, runCountStream);
}
...
function executePhase(phaseBlock) {
	...
	/*
		buildBusArray -> buildCombine -> pushOnce ->
	*/
	...
	var production = producing.map(function (val) {
		if (!val) {
			return null;
		}
		var prefixed = {};
		...
		return prefixed;
	}).fold(phaseBlock, function (memo, val) {
		if (val) {
			var keys = Object.keys(val);
			for (var i=0; i<keys.length; i++) {
